...
/** TODO: Confirm this is the correct way around */
#define LSBFIRST 0
#define MSBFIRST 1

void delay(int wait_ms) { usleep(1000 * wait_ms); }
 ...
 */
bool Adafruit_AS7341::begin(uint8_t i2c_address, int32_t sensor_id)
{
    if (i2c_dev)
    {
    ...
    i2c_dev = new Adafruit_I2CDevice(i2c_address, "/dev/i2c-1");
...
bool Adafruit_AS7341::_init(int32_t sensor_id)
{
    ...
    if ((chip_id.read() & 0xFC) != (AS7341_CHIP_ID << 2)) { return false; }
...
bool Adafruit_AS7341::checkReadingProgress()
{
    ...
    if (!getIsDataReady() || _readingState == AS7341_WAITING_DONE) return false;
        ...
        AS7341_WAITING_LOW)  // Check of getIsDataRead() is already done
    {
        ...
        // bool low_success = channel_data_reg.read((uint8_t
        // *)_channel_readings, 12);
        ...
        AS7341_WAITING_HIGH)  // Check of getIsDataRead() is already done
    {
        ...
        Adafruit_BusIO_Register channel_data_reg =
            Adafruit_BusIO_Register(i2c_dev, AS7341_CH0_DATA_L, 2);
        // return low_success &&			//low_success is lost
        // since it was last call
...
{
    for (int i = 0; i < 12; i++) readings_buffer[i] = _channel_readings[i];
    return true;
}
...
bool Adafruit_AS7341::enableSMUX(void)
{
    ...
    int timeOut = 1000;  // Arbitrary value, but if it takes 1000 milliseconds
...
bool Adafruit_AS7341::enableLED(bool enable_led)
{
    ...
    bool result =
        led_sel_bit.write(enable_led) && led_act_bit.write(enable_led);
...
bool Adafruit_AS7341::setLEDCurrent(uint16_t led_current_ma)
{
    ...
    if (led_current_ma > 258) { return false; }
    if (led_current_ma < 4) { led_current_ma = 4; }
    setBank(true);  // Access 0x60 0x74
...
{
    if (channel == AS7341_ADC_CHANNEL_5) { return false; }
